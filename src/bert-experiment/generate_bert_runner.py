...
def run_bert_pretraining(args):
    ...
    f.close()
    f = open("runner.sh", "w")
    f.write(session.run_wemux_session() + '\n')
    f.write(session.send_keys(
        "sudo python3 {} --session-name {} --zone {} --tpu-name {} --project-name {} --bert-folder {} "
    ...
    f.close()
    os.system("chmod 755 {}".format(runner))
    logging.info("To run bert pretraining type")
    logging.info("  $ ./{}".format(runner))
